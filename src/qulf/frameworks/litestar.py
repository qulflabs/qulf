...
def serve_qulf(auth: Qulf) -> Router:
    ...
    @get("/session")
    async def get_session(request: Request[Any, Any, Any]) -> Response[dict[Any, Any]]:
        session_data = await auth.get_session_from_cookies(request.cookies)
        if not session_data:
            raise NotAuthorizedException("Unauthorized")
    ...
    @post("/sign-out")
    async def sign_out(request: Request[Any, Any, Any]) -> Response[dict[str, Any]]:
        token = request.cookies.get(auth.config.cookies.name)
        if token:
    ...
    async def forgot_password(data: ForgotPasswordRequest) -> Response[dict[str, str]]:
        try:
    ...
    @post("/reset-password")
    async def reset_password(data: ResetPasswordRequest) -> Response[dict[str, str]]:
        try:
    ...
    @post("/verify-email")
    async def verify_email(data: VerifyEmailRequest) -> Response[dict[str, str]]:
        try:
            await auth.verify_email(data.token)
            return Response({"message": "Email verified successfully"}, status_code=200)
        except QulfException as e:
            return Response({"detail": str(e)}, status_code=400)
