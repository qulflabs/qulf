# tests/test_sqlalchemy.py
from datetime import datetime, timedelta, timezone

import pytest

from qulf.adapters.sqlalchemy import SQLAlchemyAdapter
from qulf.config import DeletionStrategy
from qulf.types import AccountCreate, UserCreate


@pytest.mark.asyncio
async def test_sqlalchemy_adapter_flow(sqlalchemy_adapter: SQLAlchemyAdapter):
    adapter = sqlalchemy_adapter

    user_data = UserCreate(
        name="DB User",
        email="db@test.com",
        username="dbu",
        password="p",
        password_confirmation="p",
    )

    user = await adapter.create_user(user_data, "fake_hashed_password")

    assert user.email == "db@test.com"

    fetched_by_email = await adapter.get_user_by_email_with_password("db@test.com")
    assert fetched_by_email is not None
    assert fetched_by_email.hashed_password == "fake_hashed_password"

    fetched_by_id = await adapter.get_user_by_id(user.id)
    assert fetched_by_id is not None

    assert await adapter.get_user_by_email("nobody@test.com") is None
    assert await adapter.get_user_by_id(999) is None

    expires = datetime.now(timezone.utc) + timedelta(days=1)
    session = await adapter.create_session(user.id, "tok123", expires)
    assert session.token == "tok123"

    fetched_sess = await adapter.get_session("tok123")
    assert fetched_sess is not None
    assert await adapter.get_session("bad_token") is None

    await adapter.delete_session("tok123")
    assert await adapter.get_session("tok123") is None


@pytest.mark.asyncio
async def test_sqlalchemy_session_validation_naive(
    sqlalchemy_adapter: SQLAlchemyAdapter,
):
    from qulf.config import QulfConfig
    from qulf.core import Qulf
    from qulf.types import UserCreate

    config = QulfConfig(
        secret_key="super_secret_test_key_that_is_at_least_32_bytes_long"
    )
    auth = Qulf(db=sqlalchemy_adapter, config=config)
    user_data = UserCreate(
        name="DB User 2",
        email="db2@test.com",
        username="dbu2",
        password="p",
        password_confirmation="p",
    )
    await auth.sign_up(user_data)
    session = await auth.sign_in("db2@test.com", "p")
    assert session is not None
    session_user = await auth.validate_session(session.token)
    assert session_user is not None
    session, user = session_user
    assert user and session is not None
    assert user.email == "db2@test.com"


@pytest.mark.asyncio
async def test_sqlalchemy_adapter_inject_columns(sqlalchemy_adapter: SQLAlchemyAdapter):
    adapter = sqlalchemy_adapter
    # Test injecting into a valid table
    # an unknown table, and a column that already exists
    custom_columns = {
        "user": {
            "custom_string": str,
            "custom_int": int,
            "custom_bool": bool,
            "email": str,  # Already exists, should be skipped
        },
        "unknown_table": {"fake_col": str},
    }

    adapter.inject_custom_columns(custom_columns)

    # Verify the custom columns were added to the user model
    assert hasattr(adapter.user_model, "custom_string")
    assert hasattr(adapter.user_model, "custom_int")
    assert hasattr(adapter.user_model, "custom_bool")


@pytest.mark.asyncio
async def test_sqlalchemy_adapter_update_user(sqlalchemy_adapter: SQLAlchemyAdapter):
    adapter = sqlalchemy_adapter
    user_data = UserCreate(
        name="Update User",
        email="update@test.com",
        username="update_u",
        password="p",
        password_confirmation="p",
    )
    user = await adapter.create_user(user_data, "hashed")

    # Success Update
    updated = await adapter.update_user(user.id, {"name": "Changed Name"})
    assert updated.name == "Changed Name"

    # Failure Update (User Not Found)
    with pytest.raises(ValueError, match="User not found"):
        await adapter.update_user(9999, {"name": "Changed"})


@pytest.mark.asyncio
async def test_sqlalchemy_adapter_sessions_extended(
    sqlalchemy_adapter: SQLAlchemyAdapter,
):
    adapter = sqlalchemy_adapter
    user_data = UserCreate(
        name="Session User",
        email="sess@test.com",
        username="sess_u",
        password="p",
        password_confirmation="p",
    )
    user = await adapter.create_user(user_data, "hashed")

    expires = datetime.now(timezone.utc)

    # Create 3 sessions for the same user
    await adapter.create_session(user.id, "tok1", expires)
    await adapter.create_session(user.id, "tok2", expires)
    await adapter.create_session(user.id, "tok3", expires)

    sessions = await adapter.get_user_sessions(user.id)
    assert len(sessions) == 3

    # Delete specific user session
    deleted = await adapter.delete_user_session(user.id, "tok1")
    assert deleted is True

    # Delete invalid user session
    deleted_bad = await adapter.delete_user_session(user.id, "bad_tok")
    assert deleted_bad is False

    # Delete all except tok2
    deleted_tokens = await adapter.delete_all_user_sessions(
        user.id, except_token="tok2"
    )
    assert len(deleted_tokens) == 1
    assert deleted_tokens[0] == "tok3"  # tok1 was already deleted, tok2 is excepted

    # Verify only tok2 remains
    sessions_left = await adapter.get_user_sessions(user.id)
    assert len(sessions_left) == 1
    assert sessions_left[0].token == "tok2"

    # Delete all remaining without exception
    final_deleted = await adapter.delete_all_user_sessions(user.id)
    assert len(final_deleted) == 1
    assert final_deleted[0] == "tok2"


@pytest.mark.asyncio
async def test_sqlalchemy_adapter_accounts(sqlalchemy_adapter: SQLAlchemyAdapter):
    adapter = sqlalchemy_adapter
    user_data = UserCreate(
        name="Account User",
        email="acc@test.com",
        username="acc_u",
        password="p",
        password_confirmation="p",
    )
    user = await adapter.create_user(user_data, "hashed")

    account_data = AccountCreate(
        user_id=user.id,
        account_id="gh_123",
        provider_id="github",
        access_token="access_tok",
        refresh_token="refresh_tok",
        expires_at=datetime.now(timezone.utc),
        scope="read:user",
        id_token="id_tok",
    )

    created_account = await adapter.create_account(account_data)
    assert created_account.provider_id == "github"
    assert created_account.account_id == "gh_123"

    # valid account
    fetched = await adapter.get_account_by_provider("github", "gh_123")
    assert fetched is not None
    assert fetched.user_id == user.id

    # invalid account
    not_fetched = await adapter.get_account_by_provider("github", "wrong_id")
    assert not_fetched is None


@pytest.mark.asyncio
async def test_sqlalchemy_hard_deletes_and_misses(
    sqlalchemy_adapter: SQLAlchemyAdapter,
):
    adapter = sqlalchemy_adapter
    # Hard Delete
    await adapter.delete_user("fake-id", DeletionStrategy.HARD)
    # misses returning None
    assert await adapter.get_user_by_email_with_password("nobody@nowhere.com") is None
    assert await adapter.get_user_by_id_with_password("fake-id") is None

    user = await adapter.create_user(
        UserCreate(
            name="a",
            email="soft@test.com",
            username="softu",
            password="p",
            password_confirmation="p",
        ),
        "hashed",
    )

    u1 = await adapter.get_user_by_id_with_password(str(user.id))
    assert u1 is not None and u1.hashed_password == "hashed"

    u2 = await adapter.get_user_by_email_with_password("soft@test.com")
    assert u2 is not None and u2.hashed_password == "hashed"

    # the soft delete
    await adapter.delete_user(str(user.id), DeletionStrategy.SOFT)

    # it soft deleted
    deleted_user = await adapter.get_user_by_id(str(user.id))
    assert deleted_user is not None
    assert deleted_user.deleted_at is not None


@pytest.mark.asyncio
async def test_sqlalchemy_rbac_management(
    sqlalchemy_adapter: SQLAlchemyAdapter,
):
    adapter = sqlalchemy_adapter

    # 1. PERMISSIONS
    await adapter.create_permission("read:users", "Read users")
    await adapter.create_permission("write:users", "Write users")

    fetched_perm = await adapter.get_permission_by_name("read:users")
    assert fetched_perm is not None
    assert fetched_perm.name == "read:users"

    assert await adapter.get_permission_by_name("non_existent") is None

    # 2. ROLES
    await adapter.create_role("admin", "Admin role")
    await adapter.create_role("user", "User role")

    fetched_role = await adapter.get_role_by_name("admin")
    assert fetched_role is not None
    assert fetched_role.name == "admin"

    assert await adapter.get_role_by_name("non_existent") is None

    # 3. GRANT PERMISSIONS TO ROLES
    await adapter.grant_permission_to_role("admin", "read:users")
    await adapter.grant_permission_to_role("admin", "write:users")
    await adapter.grant_permission_to_role("user", "read:users")

    # Coverage: Trigger the `except Exception: pass` block by assigning again
    await adapter.grant_permission_to_role("admin", "read:users")

    with pytest.raises(ValueError, match="Role 'fake_role' does not exist."):
        await adapter.grant_permission_to_role("fake_role", "read:users")

    with pytest.raises(ValueError, match="Permission 'fake_perm' does not exist."):
        await adapter.grant_permission_to_role("admin", "fake_perm")

    # 4. USER CREATION
    from qulf.types import UserCreate

    user = await adapter.create_user(
        UserCreate(
            name="SQLAlchemy RBAC",
            email="sqla_rbac@test.com",
            username="sqlarbac",
            password="p",
            password_confirmation="p",
        ),
        "hash",
    )

    # New user has no roles/permissions
    assert await adapter.get_user_roles(user.id) == []
    assert await adapter.get_user_permissions(user.id) == []

    # 5. ASSIGN ROLES TO USER
    await adapter.assign_role_to_user(user.id, "admin")
    await adapter.assign_role_to_user(user.id, "user")

    # Coverage: Trigger the `except Exception: pass` block by assigning again
    await adapter.assign_role_to_user(user.id, "admin")

    with pytest.raises(ValueError, match="Role 'fake_role' does not exist."):
        await adapter.assign_role_to_user(user.id, "fake_role")

    # 6. FETCH USER ROLES & PERMISSIONS
    roles = await adapter.get_user_roles(user.id)
    role_names = [r.name for r in roles]
    assert "admin" in role_names
    assert "user" in role_names

    permissions = await adapter.get_user_permissions(user.id)
    perm_names = [p.name for p in permissions]
    assert "read:users" in perm_names
    assert "write:users" in perm_names

    # 7. REMOVE ROLE FROM USER
    await adapter.remove_role_from_user(user.id, "admin")

    updated_roles = await adapter.get_user_roles(user.id)
    updated_role_names = [r.name for r in updated_roles]
    assert "admin" not in updated_role_names
    assert "user" in updated_role_names

    updated_permissions = await adapter.get_user_permissions(user.id)
    updated_perm_names = [p.name for p in updated_permissions]
    assert "write:users" not in updated_perm_names
    assert "read:users" in updated_perm_names
