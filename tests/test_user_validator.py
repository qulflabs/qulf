...
class TestUserValidator:
    @pytest.fixture
    def valid_user(self) -> UserWithPassword:
    ...
    def test_validator_successful_chain(self, valid_user: UserWithPassword) -> None:
        user = (
    ...
    def test_validator_missing_user(self) -> None:
        with pytest.raises(UserNotFoundError):
            UserValidator(None).exists()

    def test_validator_deactivated_user(self, valid_user: UserWithPassword) -> None:
    ...
    def test_validator_unverified_user(self, valid_user: UserWithPassword) -> None:
        valid_user.email_verified_at = None
        with pytest.raises(UserEmailNotVerifiedError):
            UserValidator(valid_user).verified()

    def test_validator_password_login_disabled(
        self, valid_user: UserWithPassword
    ) -> None:
    ...
    def test_validator_hidden_message(self, valid_user: UserWithPassword) -> None:
        valid_user.deleted_at = datetime.now(timezone.utc)
    ...
    def test_validator_ensure_predicate(self, valid_user: UserWithPassword) -> None:
